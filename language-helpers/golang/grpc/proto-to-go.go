...
func (self *goGrpcParser) protoToGo(i *convertingInput) (*convertingValue, error) {
	...
	if t.Type == schemas.TypeType_Int {
		...
		if t.Optional {
			...
			prepareOptional, err := self.templateManager.Parse("input-prop-optional", &templates.InputPropOptionalTemplInput{
				...
				Type:                 "*" + pbType,
				ValueToAssign:        fmt.Sprintf("int(*%s)", oi.VarToConvert),
				NeedsPointer:         true,
			})
			if err != nil {
				return nil, err
			...
			val.ProtoType = pbType
			val.Value = fmt.Sprintf("int(%s)", oi.VarToConvert)
	...
	if t.Type == schemas.TypeType_Enum {
		...
		enumConvertFuncName := fmt.Sprintf("%s%sonvertPbTo%s(*%s)", pkg, firstLetter, enum.GolangName, oi.VarToConvert)
